    ...
    public static ArrayList<Earthquake> extractEarthquakes(String JSONString) {
        ...
        try {
            ...
            for (int i = 0; i < features.length(); ++i) {
                ...
                int offsetPosition = place.indexOf("of");
                String offsetLocation, primaryLocation;
                if (offsetPosition == -1) {
                    offsetLocation = "Near the";
                    primaryLocation = place;
                } else {
                    offsetLocation = place.substring(0, offsetPosition + 3);
                    primaryLocation = place.substring(offsetPosition + 3, place.length());
                }
...
//    }
    ...
    public static String makeHTTPRequest(URL url) throws IOException{
        ...
        try{
            ...
            Log.e(LOG_TAG, "Problem  retreiveing the JSON response", exception);
        }
        finally {
    ...
    public static ArrayList<Earthquake> fetchEarthquakeData(String url){
        ...
        try{
            ...
            Log.e(QueryUtils.LOG_TAG, "Error with Closing Input Stream", e);
        }

        ArrayList<Earthquake> results = extractEarthquakes(JSONResponse);
        return results;
    }
